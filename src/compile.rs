...
fn compile_binary_primitive(stack_index: i64, expr: Prim2) -> Vec<Directive> {
    match expr {
        Prim2::Plus => vec![
...
fn compile_unary_primitive(expr: Prim1) -> Vec<Directive> {
    match expr {
        Prim1::Add1 => vec![Add(Reg(X0), operand_of_num(1))],
        Prim1::Not => [vec![Cmp(Reg(X0), operand_of_bool(false))], zf_to_bool()].concat(),
        Prim1::Sub1 => vec![Sub(Reg(X0), operand_of_num(1))],
        Prim1::IsNum => [
            vec![And(Reg(X0), Imm(NUM_MASK)), Cmp(Reg(X0), Imm(NUM_TAG))],
            zf_to_bool(),
        ]
        .concat(),
        Prim1::IsZero => [vec![Cmp(Reg(X0), operand_of_num(0))], zf_to_bool()].concat(),
    }
}
pub fn compile_expr(
